...
PC = "PCONST"
dictPCONST = {}
whitelistPCONST = ["EEOO","OOEE","EEXX","OOXX","XXEE","XXOO"]
...
for row in cur:
	...
	vRF = float(row[3])
	vRT = float(row[4])
	pcon = CalculatePCONST(vLF, vLT, vRF, vRT)
	row[0] = pcon
	if not pcon in whitelistPCONST and not pcon in dictPCONST:
		dictPCONST[pcon] = pcon
	cur.updateRow(row)

row, cur = None, None
for i in dictPCONST:
	ct = 0
	with arcpy.da.SearchCursor(lyr, PC) as cur:
		for row in cur:
			if row[0] == i:
				ct = ct + 1
	msg(i + ": " + str(ct))
