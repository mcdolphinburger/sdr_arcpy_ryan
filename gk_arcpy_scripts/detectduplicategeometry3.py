...
Script: dtectduplicategeometry3.py
Created: 20140715
Modified: 20170518
...
try:
    msg("Scanning for duplicate geometries.")
    cursorfields = []
    ...
    for row in cur:
        ...
        if res == "--":
            ...
            arcpy.SelectLayerByLocation_management(lyr, "ARE_IDENTICAL_TO", row[1], "", "NEW_SELECTION")
            ct = GetSelectionCount(lyr)
            if ct > 1:
                msg("Object ID: " + str(id) + "        Selection Count: " + str(ct))
                dupcount = dupcount + 1
                row[2] = resval + "-" + str(dupcount)
                cur.updateRow(row)
