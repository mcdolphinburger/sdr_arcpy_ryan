...
doParse = arcpy.GetParameter(3)					# Boolean on whether to do the parse or just add a message
...
# -------------------------------------------------------------------------------------------------------------	
msg("\n\nParsing street types from field " + fldN + " and putting them in field " + fldT + ".\n\n")
...
for row in cur:
    x = row[0]
    q = ParseStreetType(x)
    if q[1] != "" and len(row[1].strip()) > 0:
        msg("  " + str(row[2]) + ": Already has a street type, skipping")
    elif q[1] != "":
		i = i + 1
		msg("  " + x + " --->  " + q[0] + " | " + q[1])
		row[0] = q[0]
